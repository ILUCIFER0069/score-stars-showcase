...
import type { ParticipantWithHistory, DatabasePointHistory } from '@/types/database';
import { CalendarIcon } from 'lucide-react';

interface ParticipantDetailProps {
  participant: ParticipantWithHistory | null;
  isOpen: boolean;
  onClose: () => void;
}
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          {participant.history && participant.history.length > 0 ? (
            <div className="space-y-4">
              {participant.history.map((item: DatabasePointHistory) => (
            ...
            <div className="py-8 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">History can be added through the admin panel</p>
            </div>
          )}
        </div>
      </DialogContent>
